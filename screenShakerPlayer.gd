...
# Settings
export var INTENSITY = 5  # (0 - 20)s used as the range in the tutorial
export var DURATION = .1  # (0 - .5)s used as the range in the tutorial
...
func _ready():
	...
	#  - Get the camera from player
	camera = get_parent().find_node("Camera2D")
	assert(camera != null, "Shake cannot find camera!")
